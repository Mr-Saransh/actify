...
export function GoalIntelligence({ goal }: GoalIntelligenceProps) {
    ...
    return (
        <Card className="bg-zinc-950 border-zinc-800 border-b-2 rounded-none md:rounded-lg mb-4 md:mb-6">
            <CardContent className="p-3 md:p-4 lg:p-6 grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                {/* Type */}
                <div className="flex flex-col gap-1 border-r border-zinc-900 last:border-0 pr-3 md:pr-4">
                    <span className="text-[9px] md:text-[10px] text-zinc-500 uppercase tracking-widest flex items-center gap-1">
                        <Target className="h-2.5 w-2.5 md:h-3 md:w-3" /> Type
                    </span>
                    <span className="font-mono text-xs md:text-sm font-bold text-white uppercase">{goal.type}</span>
                </div>
                ...
                <div className="flex flex-col gap-1 border-r border-zinc-900 last:border-0 pr-3 md:pr-4">
                    ...
                    <div className="flex items-baseline gap-2">
                        ...
                        <span className="text-[9px] md:text-[10px] text-zinc-600 font-mono hidden md:inline">
                            (-{daysRemaining}d)
                        </span>
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-1 border-r border-zinc-900 last:border-0 pr-3 md:pr-4">
                    ...
                    <span className="font-mono text-xs md:text-sm font-bold text-white">
                        {completed} <span className="text-zinc-600">/ {totalTasks}</span>
                    </span>
                </div>
                ...
                <div className="flex flex-col gap-1 pr-3 md:pr-4">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="flex-1 h-1 md:h-1.5 bg-zinc-900 rounded-full overflow-hidden">
                            <div className="h-full bg-white" style={{ width: `${progress}%` }} />
                        </div>
                        <span className="font-mono text-xs md:text-sm font-bold text-white">{progress}%</span>
                    </div>
                </div>
            </CardContent>
        </Card >
