...
export function FailurePanel({ metrics }: FailurePanelProps) {
    return (
        <div className="bg-zinc-950/50 border border-red-900/30 rounded-lg p-4 font-mono text-sm relative overflow-hidden h-full flex flex-col">
            {metrics.riskLevel === 'CRITICAL' && (
                <div className="absolute inset-0 bg-red-900/10 animate-pulse pointer-events-none" />
            )}

            <div className="flex items-center gap-2 mb-4 text-red-500 font-bold uppercase tracking-widest border-b border-red-900/30 pb-2">
